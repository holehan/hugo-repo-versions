...
SNAP_REMOTE="$(curl -s -H 'X-Ubuntu-Series: 16' 'https://api.snapcraft.io/api/v1/snaps/details/hugo' | jq -r '.version' | grep -Eo -m 1 'v[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}' | sed 's/^v//g')"
SNAP_LOCAL="$(curl -s https://hugo-repo-versions.netlify.app/snap/index.json | jq -r '.data.version')"
SNAP_EXTENDED_REMOTE="$(curl -s -H 'X-Ubuntu-Series: 16' 'https://api.snapcraft.io/api/v1/snaps/details/hugo?channel=extended' | jq -r '.version' | grep -Eo -m 1 'v[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}' | sed 's/^v//g')"
